  ...
  return (
    <section className="py-16 bg-gray-50" id="surgeries">
      <div className="max-w-6xl mx-auto px-6">
        {/* Heading */}
        <h2 className="text-3xl md:text-4xl font-bold mb-10 text-center text-gray-800 animate-fadeInUp opacity-0">
          <span className="text-[#0089FF] text-4xl">Surgeries</span> Offered (General + Laparoscopic)
        </h2>
        <div className="w-20 h-1 bg-[#0089FF] mx-auto mb-10 rounded-full animate-fadeInUp opacity-0 [animation-delay:0.1s]"></div>

        {/* Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6 animate-fadeInUp opacity-0 [animation-delay:0.2s]">
          {surgeries.map((surgery, idx) => {
            ...
            return (
              <Link
                key={idx}
                href={surgery.slug}
                className={`cursor-pointer p-6 bg-white border border-gray-200 rounded-xl text-center font-medium text-gray-800 shadow-md transition-transform duration-300 hover:scale-105 hover:shadow-lg hover:border-[#0089FF] hover:text-white hover:bg-[#0089FF] hover:shadow-[#0089FF]/30 animate-fadeInScale opacity-0 [animation-delay:${0.3 + idx * 0.07}s]`}
              >
